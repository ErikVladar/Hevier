...
<body class="font-[Kaushan] antialiased">
    ...
    <div x-data="gallerySlideshow({{ Js::from($images) }})">
        <div
            ...
            <div id="gallery" class="rounded-t-3xl bg-white/30 z-40">
                <!-- Gallery Grid -->
                <section class="py-20">
                    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                            ...
                            @foreach ($images as $index => $image)
                                <button @click="show({{ $index }})" class="group">
                                    <div
                                        class="relative overflow-hidden rounded-2xl shadow-lg transform hover:scale-[1.03] transition duration-300">
        ...
        <!-- ✨ Slideshow Overlay with Animation ✨ -->
        <div x-show="open"
            x-transition:enter="fade-scale-enter"
            x-transition:enter-active="fade-scale-enter-active"
            x-transition:leave="fade-scale-leave"
            x-transition:leave-active="fade-scale-leave-active"
            class="fixed inset-0 bg-black bg-opacity-90 z-[1000] flex items-center justify-center p-6"
            x-cloak>
            <!-- Close -->
            <button @click="close"
                class="absolute top-6 right-8 text-white text-4xl hover:text-red-400 transition duration-200">&times;</button>
            ...
            <div
                ...
                <div class="w-full md:w-1/2 h-full flex items-center justify-center relative">
                    <img :src="currentSlide.image" alt=""
                        class="object-contain h-full max-h-full rounded-lg shadow-lg transition-all duration-500 ease-in-out"
                        :class="animateImage ? 'opacity-100 scale-100' : 'opacity-0 scale-95'">
                </div>
        ...
        function gallerySlideshow(images) {
            return {
                ...
                triggerImageAnimation() {
                    ...
                    setTimeout(() => this.animateImage = true, 50);
                },
            };
        }
    </script>
